...
jQuery.fn.contextPopup = function(menuData) {
	...
	var settings = {
		...
		title: '',
		items: [],
		// ETUDES: added settings.event to allow for 'click' or 'contextmenu'
		event: 'contextmenu'
	};
  ...
  // Build popup menu HTML
    ...
    settings.items.forEach(function(item) {
      if (item) {
        var rowCode = '<li><a href=""><span></span></a></li>'; // ETUDES: removed the # in the href
        // if(item.icon)
        //   rowCode += '<img>';
        ...
        if (item.isEnabled != undefined && !item.isEnabled()) {
            ...
            row.find('a').click(function () { return item.action(e); }); // ETUDES: added the "return item.action"
        } else if (item.url) {			// ETUDES: added "url", "download" options
        	if (item.download) {
        		row.find('a').attr("download", item.download);
        		row.find('a').attr("href", item.url + "?attachment=1");
        	} else {
            	row.find('a').attr("href", item.url).attr("target","_blank");        		
        	}
        }
